...
`docs/reference/` documents CLI verbs (parameter tables), not Swift declarations, and its
verb-registry-vs-docs drift is already guarded by `Scripts/verb-check.sh` /
`.github/workflows/verbs.yml` — that mechanism predates this policy and already does for the
verb surface what the ecosystem's wider proposal asks docs-drift tooling to do generally.
...
Why: this repo is the ecosystem's pilot for the wider code-style policy, chosen because it's small
enough (48 files, ~10k lines) to sweep into full compliance in one PR rather than needing a gradual
adoption mechanism. Full research and rationale:
...
Filed and tracked as [OCCTSwiftScripts#114](https://github.com/SecondMouseAU/OCCTSwiftScripts/issues/114).
