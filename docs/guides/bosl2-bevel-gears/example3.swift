// BOSL2 `bevel_gear()` Example 3: a 14-tooth pinion and a 28-tooth gear placed by hand
// rather than by anchor, with cylindrical (`cone_backing=false`) backing on the gear.
//
// One of three scripts behind `docs/guides/bosl2-bevel-gears.md`, which shows this next to
// the OpenSCAD original. The reference OpenSCAD source is `example3.scad` in this folder.
//
// Run:
//   swift run occtkit run docs/guides/bosl2-bevel-gears/example3.swift \
//       --format brep --output /tmp/bosl2-ex3
...
// ══════════════════════════════════════════════════════════════════════════════════════
// Example 3: manual spacing of pinion and gear, with conical backing turned off.
//
// BOSL2 gears.scad:2415-2430, verbatim:
//
...
//     }
//
// BOSL2 renders this as a four-frame animation; `$t = 0` is reproduced, so both `yrot` and
// `zrot` are the identity. The two new things versus Example 2 are explicit placement
// (default anchor="pitchbase" plus a translation, rather than anchor="apex") and
// `cone_backing=false`, which attaches a plain cylinder instead of continuing the taper.
// ══════════════════════════════════════════════════════════════════════════════════════
...
let ctx = ScriptContext(exportSTEP: true, metadata: ManifestMetadata(
    name: "BOSL2 bevel_gear() Example 3: manual spacing, cylindrical backing",
    source: "OCCTSwiftScripts docs/guides/bosl2-bevel-gears/example3.swift",
    tags: ["bosl2", "gear", "bevel", "spiral", "cylindrical-backing", "docs"]
))
...
let pinion = buildGear(
    GearParams(teeth: t1, mateTeeth: Double(t2), slices: 12, shaftDiam: 5),
    label: "Example 3 pinion: bevel_gear(circ_pitch=5, teeth=14, mate_teeth=28, shaft_diam=5, slices=12, orient=FWD)")

let gear = buildGear(
    GearParams(teeth: t2, mateTeeth: Double(t1), slices: 12, rightHanded: true,
               backing: 3, coneBacking: false, shaftDiam: 5),
    label: "Example 3 gear: bevel_gear(circ_pitch=5, teeth=28, mate_teeth=14, right_handed=true, shaft_diam=5, slices=12, backing=3, spin=180/28, cone_backing=false)")

// ── Check 1: the backing really is cylindrical, not conical. ──────────────────────────
// `buildGear` has already compared the measured volume increase against the analytic solid
// (a cylinder here, a frustum when cone_backing is true) and would have stopped if they
// disagreed by more than 0.5%. What is left to assert is that the two forms are actually
// different, which is what makes `cone_backing=false` a feature rather than a no-op: the
// conical form for this gear would taper from r0 by tan(pitch_angle-90)*backing per unit
// depth. A cone_backing=false that silently kept the taper would fail this.
let backingDepth = gear.appliedBacking
let r0 = meridianRZ(u: 1, R: gear.g.rootR, g: gear.g).x
let coneFarRadius = r0 + tan(gear.g.pitchAngle - .pi / 2) * backingDepth
let cylinderVolume = Double.pi * r0 * r0 * backingDepth
let coneVolume = (Double.pi * backingDepth / 3) * (r0 * r0 + r0 * coneFarRadius + coneFarRadius * coneFarRadius)
print("check 1, cylindrical backing: r0=\(r0) depth=\(backingDepth) " +
      "cylinder=\(cylinderVolume) mm3 vs the conical form's \(coneVolume) mm3 (far radius \(coneFarRadius))")
guard abs(cylinderVolume - coneVolume) / cylinderVolume > 0.05 else {
    fatalError("Example 3: cone_backing=false is indistinguishable from cone_backing=true here, so this example proves nothing")
}
print("  PASS: the two backing forms differ by \(abs(cylinderVolume - coneVolume) / cylinderVolume * 100)% of the cylinder volume")

// ── Place both members exactly as BOSL2 does, at $t = 0. ──────────────────────────────
// Pinion: default anchor="pitchbase", orient=FWD, then back(pitch_radius(circ_pitch, 28)).
// Gear:   default anchor="pitchbase", spin=180/28, then down(pitch_radius(circ_pitch, 14)).
let pinionPlaced = movedBy(orientedFWD(anchoredAtPitchbase(pinion.shape, pinion.frames)),
                           SIMD3(0, pitchRadius(teeth: Double(t2)), 0))
let gearPlaced = movedBy(spun(anchoredAtPitchbase(gear.shape, gear.frames), degrees: 180.0 / Double(t2)),
                         SIMD3(0, 0, -pitchRadius(teeth: Double(t1))))

for (label, s) in [("pinion", pinionPlaced), ("gear", gearPlaced)] {
    let h = HealthSnapshot(s)
    guard h.solidCount >= 1 else { fatalError("Example 3: placed \(label) is not a solid (solidCount=\(h.solidCount))") }
    let b = s.bounds
    print("placed \(label): bounds min=(\(b.min.x), \(b.min.y), \(b.min.z)) max=(\(b.max.x), \(b.max.y), \(b.max.z)) volume=\(h.volume)")
}

// ── Check 2: both pitch-cone apexes land on the world origin. ─────────────────────────
// This one is real evidence, unlike the same statement in Example 2. Nothing here was
// positioned by its apex: both members were anchored on their PITCHBASE and then moved by
// BOSL2's own pitch_radius values, one along +Y and one along -Z. They only meet at the
// origin because a gear's apex height above its pitch base equals its mate's pitch radius.
// Get that formula wrong and the two apexes land in different places.
let pinionApexAbove = pinion.frames.apex - pinion.frames.pitchbase
let gearApexAbove = gear.frames.apex - gear.frames.pitchbase
// Pinion: (0,0,h) -> orient FWD (xrot 90) -> (0,-h,0) -> back(pr(t2)) -> (0, pr(t2)-h, 0).
let pinionWorldApex = SIMD3<Double>(0, pitchRadius(teeth: Double(t2)) - pinionApexAbove, 0)
// Gear: (0,0,h) -> spin about Z leaves it -> down(pr(t1)) -> (0, 0, h - pr(t1)).
let gearWorldApex = SIMD3<Double>(0, 0, gearApexAbove - pitchRadius(teeth: Double(t1)))
print("check 2, world apex: pinion -> \(pinionWorldApex), gear -> \(gearWorldApex)")
guard simd_length(pinionWorldApex) < 1e-9, simd_length(gearWorldApex) < 1e-9 else {
    fatalError("Example 3: the two apexes do not meet at the origin (pinion \(pinionWorldApex), gear \(gearWorldApex))")
}
print("  PASS: both apexes at the world origin, reached through pitchbase anchors and pitch_radius alone")

// ── Check 3: the meshed pair meshes rather than collides. ─────────────────────────────
// BOSL2's own `spin=180/t2` idiom is what interleaves the teeth. This construction derives
// its tooth phase from its own tooth-SPACE cutter rather than from BOSL2's tooth profile, so
// the two conventions could differ by half a pitch and drive tooth into tooth. The boolean
// intersection of the two positioned solids is what tells us, and the natural yardstick is
// one tooth space: a half-pitch error buries a whole tooth in a whole gap, which costs a
// large fraction of that volume. The threshold is 5% of one tooth space, so this check fails
// on a phase error while tolerating the small overlap that faceting alone produces at zero
// backlash (both flanks are polylines, so each tooth is marginally fatter than the exact
// involute and the pair cannot be exactly tangent).
let toothSpace = min(pinion.toothSpaceVolume, gear.toothSpaceVolume)
let interferenceLimit = toothSpace * 0.05
if let overlap = pinionPlaced.intersection(gearPlaced), let v = overlap.volume {
    print("check 3, interference: intersection volume = \(v) mm3 = \(v / toothSpace * 100)% of one tooth space (\(toothSpace) mm3), limit \(interferenceLimit) mm3")
    guard v < interferenceLimit else {
        fatalError("Example 3: the placed pair interferes by \(v) mm3, over the \(interferenceLimit) mm3 limit; the tooth phase is wrong")
    }
    print("  PASS: the pair meshes; the residual overlap is faceting, not a phase error")
} else {
    print("check 3, interference: intersection empty (no shared volume)")
    print("  PASS")
}

try ctx.add(pinionPlaced, color: C.copper, name: "pinion 14t spiral, mate 28t, shaft_diam=5, orient=FWD, back(pr(28))")
try ctx.add(gearPlaced, color: C.steel, name: "gear 28t spiral right_handed, mate 14t, shaft_diam=5, backing=3 cylindrical, spin=180/28, down(pr(14))")
try ctx.emit(description: "BOSL2 bevel_gear() Example 3: 14t + 28t spiral bevel pair, explicit placement, cylindrical backing")
