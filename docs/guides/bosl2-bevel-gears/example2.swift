// BOSL2 `bevel_gear()` Example 2: a 16-tooth pinion meshing with a 28-tooth spiral bevel
// gear at a 90 degree shaft angle, both placed by their pitch-cone apex.
//
// One of three scripts behind `docs/guides/bosl2-bevel-gears.md`, which shows this next to
// the OpenSCAD original. The reference OpenSCAD source is `example2.scad` in this folder.
//
// Run:
//   swift run occtkit run docs/guides/bosl2-bevel-gears/example2.swift \
//       --format brep --output /tmp/bosl2-ex2
...
//
// BOSL2 gears.scad:2405-2414, verbatim:
//
//     t1 = 16; t2 = 28;
...
//     );
//
// A meshing pair at a 90 degree shaft angle. Default spiral=35, so both members get spiral
// teeth; the 28-tooth gear is right-handed and gets 3mm of conical backing, which is what
// stops it being too thin.
// ══════════════════════════════════════════════════════════════════════════════════════
...
let ctx = ScriptContext(exportSTEP: true, metadata: ManifestMetadata(
    name: "BOSL2 bevel_gear() Example 2: spiral bevel gear and pinion",
    source: "OCCTSwiftScripts docs/guides/bosl2-bevel-gears/example2.swift",
    tags: ["bosl2", "gear", "bevel", "spiral", "meshing", "docs"]
))
...
let pinion = buildGear(
    GearParams(teeth: t1, mateTeeth: Double(t2), slices: 12),
    label: "Example 2 pinion: bevel_gear(circ_pitch=5, teeth=16, mate_teeth=28, slices=12, anchor=\"apex\", orient=FWD)")

let gear = buildGear(
    GearParams(teeth: t2, mateTeeth: Double(t1), slices: 12, rightHanded: true,
               backing: 3, coneBacking: true),
    label: "Example 2 gear: bevel_gear(circ_pitch=5, teeth=28, mate_teeth=16, right_handed=true, slices=12, anchor=\"apex\", backing=3, spin=180/28)")

// ── Check 1: the teeth are SPIRAL, not zerol and not straight. ────────────────────────
// spiral=35 puts the cutter arc off to one side, so the trace's angular offset runs
// monotonically from one sign to the other across the face width. Zerol returns to the same
// sign at both ends (Example 1), straight gives three angles at zero. This check fails for
// either of those.
for member in [pinion, gear] {
    let a = traceAngles(member.cutter)
    print("check 1, spiral trace [\(member.params.teeth)t]: \(a.start)deg, \(a.mid)deg, \(a.end)deg")
    guard a.start * a.end < 0, abs(a.start - a.end) > 10 else {
        fatalError("Example 2: tooth trace on the \(member.params.teeth)t member is not a spiral (\(a))")
    }
}
print("  PASS: both members have a monotonic sign-changing lengthwise trace")

// ── Check 2: apex height above the pitch base equals the MATE's pitch radius. ─────────
// The independently-derivable identity from OCCTSwiftScripts#88, and the check that can
// actually fail on a wrong apex formula. 16 against 28 makes the two numbers different, so
// unlike Example 1's miter pair this instance is not self-satisfying.
for member in [pinion, gear] {
    let above = member.frames.apex - member.frames.pitchbase
    let matePR = pitchRadius(teeth: member.params.mateTeeth)
    print("check 2, apex height [\(member.params.teeth)t]: apex-pitchbase=\(above) " +
          "vs pitch_radius(mate=\(member.params.mateTeeth))=\(matePR) diff=\(abs(above - matePR))")
    guard abs(above - matePR) < 1e-9 else {
        fatalError("Example 2: apex height \(above) on the \(member.params.teeth)t member does not match pitch_radius(mate)=\(matePR)")
    }
}
print("  PASS")

// ── Place both members exactly as BOSL2 does. ─────────────────────────────────────────
// BOSL2 applies anchor, then spin, then orient (attachments.scad `reorient`), and the order
// matters: spinning after the orient would rotate about the already-tilted axis rather than
// the gear's own manufacturing axis.
let pinionPlaced = orientedFWD(anchoredAtApex(pinion.shape, pinion.frames))
let gearPlaced = spun(anchoredAtApex(gear.shape, gear.frames), degrees: 180.0 / Double(t2))

for (label, s) in [("pinion", pinionPlaced), ("gear", gearPlaced)] {
    let h = HealthSnapshot(s)
    guard h.solidCount >= 1 else { fatalError("Example 2: placed \(label) is not a solid (solidCount=\(h.solidCount))") }
    let b = s.bounds
    print("placed \(label): bounds min=(\(b.min.x), \(b.min.y), \(b.min.z)) max=(\(b.max.x), \(b.max.y), \(b.max.z)) volume=\(h.volume)")
}

// NOT EVIDENCE, recorded as such: both apexes now sit at the world origin, but that is true
// by construction of `anchoredAtApex`, which translates each gear by the negative of its own
// apex height. It would still hold if the apex formula were wrong. Check 2 above is the one
// that can fail. Example 3 reaches the same assembly through pitchbase anchors and BOSL2's
// pitch_radius values instead, where apex coincidence IS a real result.
...
if let overlap = pinionPlaced.intersection(gearPlaced), let v = overlap.volume {
    ...
    guard v < interferenceLimit else {
        fatalError("Example 2: the placed pair interferes by \(v) mm3, over the \(interferenceLimit) mm3 limit; the tooth phase is wrong")
    }
...
try ctx.add(pinionPlaced, color: C.copper, name: "pinion 16t spiral, mate 28t, anchor=apex, orient=FWD")
try ctx.add(gearPlaced, color: C.steel, name: "gear 28t spiral right_handed, mate 16t, anchor=apex, backing=3, spin=180/28")
try ctx.emit(description: "BOSL2 bevel_gear() Example 2: 16t + 28t spiral bevel pair at a 90 degree shaft angle")
