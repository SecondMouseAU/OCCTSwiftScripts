...
extension Composer {
    ...
    public struct DrawingComponent: Sendable {
        ...
        /// before projection (its assembled position).
        ///
        /// `nil` → project the shape as-is (already positioned). Mirrors
    ...
    public static func render(
    ...
    ) throws -> DrawingComposerResult {
        guard !components.isEmpty else {
            throw DrawingComposerError.shapeBuildFailed(
                "render(spec:components:) needs at least one component")
        }
        ...
        for view in spec.views {
            ...
            for (_, item) in comps {
                writer.collectFromDrawing(
                    item.drawing.transformed(translate: p.translate, scale: p.scale))
            }
        ...
        if let anchorComps = perView[anchorName], let p = placed[anchorName] {
            let itemByIdx = Dictionary(
                anchorComps.map { ($0.idx, $0.item) },
            ...
            for key in order {
                ...
                // 12 mm clear of the part, sheet space
                let balloon = SIMD2(target.x, target.y + 12)
                drawBalloon(
                    into: writer, number: g.item.number, centre: balloon, target: target, radius: 4)
            }
        }
    ...
    /// flattening it to its positioned leaf parts.
    ///
    /// Leaf names drive the parts list; identical names merge into one BOM row
    ...
    public static func render(spec: DrawingSpec, document: Document) throws -> DrawingComposerResult
    {
        ...
        guard !components.isEmpty else {
            throw DrawingComposerError.shapeBuildFailed(
                "assembly document has no leaf geometry to draw")
        }
    ...
    static func applyAutoCenterAnnotations(
        perView: [String: [(idx: Int, item: ViewItem)]],
        positioned: [Shape],
        spec: DrawingSpec
    ) {
    ...
    /// the part.
    ///
    /// Mirrors the upstream `DrawingAnnotation.balloon` rendering but in sheet
    /// coordinates so the balloon doesn't scale with the drawing.
    static func drawBalloon(
